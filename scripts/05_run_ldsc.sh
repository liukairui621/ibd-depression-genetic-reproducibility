...
M="$ROOT/data/munged"
OUT="$ROOT/results/global"
LOG="$ROOT/logs/05_run_ldsc.log"
...
printf '%s\n' "${DEP_TRAITS[@]}" "${IBD_TRAITS[@]}" | sort -u > "$ROOT/provenance/layer1_trait_ids.txt"
...
export -f run_h2
printf '%s\n' "${DEP_TRAITS[@]}" "${IBD_TRAITS[@]}" | sort -u | xargs -n1 -P4 bash -c 'run_h2 "$0"'

PAIR_FILE="$ROOT/provenance/layer1_pair_manifest.tsv"
...
for ((a=0; a<${#DEP_TRAITS[@]}; a++)); do
  for ((b=a+1; b<${#DEP_TRAITS[@]}; b++)); do
    printf 'within_depression\t%s\t%s\n' "${DEP_TRAITS[$a]}" "${DEP_TRAITS[$b]}" >> "$PAIR_FILE"
  done
done
...
export -f run_rg
tail -n +2 "$PAIR_FILE" | tr '\t' ' ' | xargs -n3 -P4 bash -c 'run_rg "$0" "$1" "$2"'

date --iso-8601=seconds > "$ROOT/provenance/ldsc_completed_at.txt"
